...
namespace AgileConfig.Server.Service
{
    public class ConfigService : IConfigService
    {
        ...
        public async Task<List<Config>> Search(string appId, string group, string key, string env)
        {
            ...
            //var q = dbcontext.Configs.Where(c => 1 == 1);
            //if(q.Count() == 0)
            //{
            //    return await q.ToListAsync();
            //}
            //q = q.Where(c => c.Status == ConfigStatus.Enabled && String.Equals(c.Env, env, StringComparison.OrdinalIgnoreCase));
        ...
        public async Task<List<PublishTimeline>> GetPublishTimelineHistoryAsync(string appId, string env)
        {
            ...
            var list = await dbcontext.PublishTimeline.Where(x => x.AppId == appId && x.Env == env)
                                                      .ToListAsync();
        ...
        public async Task<List<PublishDetail>> GetPublishDetailListAsync(string appId, string env)
        {
            ...
            var list = await dbcontext.PublishDetail.Where(x => x.AppId == appId && x.Env == env)
                                                    .ToListAsync();
        ...
        public async Task<List<PublishDetail>> GetConfigPublishedHistory(string configId, string env)
        {
            ...
            var list = await dbcontext.PublishDetail.Where(x => x.ConfigId == configId && x.Env == env)
                                                    .ToListAsync();
