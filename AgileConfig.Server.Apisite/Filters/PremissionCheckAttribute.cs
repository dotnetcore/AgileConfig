...
namespace AgileConfig.Server.Apisite.Filters
{
    public class PremissionCheckAttribute : ActionFilterAttribute
    {
        ...
        /// </summary>
        static Dictionary<string, Func<IDictionary<string, object>, string>> _getAppIdParamFuncs = new Dictionary<string, Func<IDictionary<string, object>, string>>
        {
            {
                "Config.Add",(args)=> { var model = args["model"];  return (model as ConfigVM)?.AppId; }
            },
            {
                "Config.Edit",(args)=> { var model = args["model"];  return (model as ConfigVM)?.AppId; }
            }
        };
        ...
        public PremissionCheckAttribute(IPermissionService premissionService, string actionName, string functionKey)
        {
        ...
        public override async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            ...
            if (_getAppIdParamFuncs.ContainsKey(_actionName))
            {
                ...
                appId = func(context.ActionArguments);
            }
