...
public class PermissionService(
    ...
    IRepository<App> appRepository) : IPremissionService
{
    ...
    private async Task<List<App>> GetUserAuthApp(string userId, string authPermissionKey)
    {
        ...
        var userAuths = await userAppAuthRepository.SearchFor(x => x.UserId == userId && x.Permission == authPermissionKey).ToListAsync();
        foreach (var appAuth in userAuths)
        {
            var app = await appRepository.SearchFor(x => x.Id == appAuth.AppId).FirstAsync();
            if (app != null)
            {
                apps.Add(app);
            }
    ...
    {
        return await appRepository.SearchFor(x => x.AppAdmin == userId).ToListAsync();
    }
