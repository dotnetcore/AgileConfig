...
{
    /// <summary>
    ///     Publish the current pending configuration items. When ids are provided, only those items are published; otherwise
    ///     all pending items are published.
    /// </summary>
    ...
    Task<(bool result, string publishTimelineId)> Publish(string appId, string[] ids, string log, string operatorr,
        string env);
