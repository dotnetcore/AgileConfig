...
public class ReactUiMiddleware
{
    ...
    {
        return context.Request.Path is { HasValue: true, Value: not null } &&
               context.Request.Path.Value.Equals("/ui", StringComparison.OrdinalIgnoreCase);
    }
    ...
    public async Task Invoke(HttpContext context)
    {
        //handle /ui request
        var filePath = "";
        if (ShouldHandleUiRequest(context)) filePath = _uiDirectory + "/index.html";
        //handle static files that Referer = xxx/ui
        if (ShouldHandleUiStaticFilesRequest(context)) filePath = _uiDirectory + context.Request.Path;
        ...
        else
        {
            ...
            {
                if (filePath.EndsWith("index.html")) await RewriteIndexHtml(filePath);
            ...
            if (context.Request.Headers.TryGetValue("If-Modified-Since", out var values))
            {
                var lastModified = uiFile.LastModified;
                if (DateTime.TryParse(values.First(), out var ifModifiedSince) && ifModifiedSince >= lastModified)
                {
