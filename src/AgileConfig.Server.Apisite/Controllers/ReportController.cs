...
public class ReportController : Controller
{
    ...
    public async Task<IActionResult> SearchServerNodeClients(string address, string appId, string env, int current,
        int pageSize)
    {
        ...
        {
            if (nodes.Any(x =>
                    x.Status == NodeStatus.Online &&
                    x.Id.ToString().Equals(address, StringComparison.CurrentCultureIgnoreCase)))
        ...
        foreach (var addr in addressess)
        {
            ...
            if (report != null && report.Infos != null) clients.AddRange(report.Infos);
        }
    ...
    public async Task<IActionResult> RemoteNodesStatus()
    {
        ...
        foreach (var serverNode in nodes)
        {
            ...
            {
                n = serverNode,
                server_status = await _remoteServerNodeProxy.GetClientsReportAsync(serverNode.Id)
            });
        }
